...
struct AppState
{
    ...
    volatile uint32_t   updateFrameCount;
    volatile uint32_t   renderFrameCount;

    ThreadMutex         mutex;
};

int Game_ThreadUpdate(void* data)
{
    ...
    {
        appState->mutex.Lock();
        if (appState->updateFrameCount < appState->renderFrameCount + GAME_FRAME_QUEUE_COUNT)
        {
            const uint32_t frameToUpdate = appState->updateFrameCount++;
            Game_Update(frameToUpdate, 0.0f, deltaTime);
        }
...
int main(const int argc, const char* argv[])
{
    ...
    appState.renderFrameCount = 0;
    appState.updateFrameCount = 0;
    appState.mutex.Create();

    int objectCount = 500000;
    Game_Setup(&spriteBatch, objectCount);
 
    ThreadDesc updateThread;
    Thread_Run(Game_ThreadUpdate, &appState, &updateThread);
    ...
    while ((appState.isRunning = Window_PollEvents()))
    {
        Renderer_Clear();

        const bool render = appState.renderFrameCount < appState.updateFrameCount;
        if (render)
        {
            const uint32_t frameToRender = appState.renderFrameCount++;
            Game_Render(frameToRender);
        }
        ...
        Renderer_DrawQuad(
            vec2(0.0f, (float)Window_GetHeight()),
            vec2(fpsTextSize.x + 10.0f, (float)Window_GetHeight() - 1.5f * fpsTextSize.y - 10.0f),
            vec3(0.0f));
        Renderer_DrawText(fpsText, vec2(5.0f, (float)Window_GetHeight() - 5.0f), vec3(0.25f, 0.5f, 1.0f));
        Renderer_DrawText(objectCountText, vec2(5.0f, (float)Window_GetHeight() - 45.0f), vec3(0.25f, 0.5f, 1.0f));
    ...
    Thread_Join(&updateThread);
    appState.mutex.Destroy();
